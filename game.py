...
def button(msg,x,y,w,h,ic,ac,action=None):  # msg = message | x = disposition axe horizontal (width) | y = disposition axe vertical (height) | ia = couleur foncé | ac = couleur claire
    ...
    if x+w > mouse[0] > x and y+h > mouse[1] > y: #automatisation des boutons : paramètre msg, x,y,h,w,ac;ic a remplacer par les données vouluent
...
def game_menu():
    ...
    while menu:
        ...
        screen.blit(TextSurf, TextRect) #apparition du texte sur l'ecran
        #fonction play_game appellée 
        button("Jouer",150,600,100,50,VERT_FONCE,VERT, play_game) # paramètre du bouton : son texte, sa position et taille, sa couleur et enfin l'action que celui-ci enclanche
        ...
        mouse = pygame.mouse.get_pos() # paramètre pour capté la souris
...
def pause_():
    ...
    while pause:
        ...
        # Composer de deux boutons: continuer qui déclanche la fonction unpause
        button("Continuer",150,450,120,50,VERT_FONCE,VERT, unpause) #bouton qui suspend la pause et reprend le jeu la ou il a été laissé
        button("Stop",550,450,100,50,ROUGE_FONCE,ROUGE,pygame.quit) # déclanche la fin de parti avec pygame.quit()
...
def gameOverP1():
    ...
    while True :
        ...
        button("Rejouer",150,100,100,50,VERT_FONCE,VERT,play_game) #bouton qui relance le jeu en faisant appel a la fonction de lancement de jeu
...
def instructions():
    ...
    while True:
        ...
        TextRect.center = ((WIDTH/2),80) #disposition centrer sur l'horizontal 
        screen.blit(TextSurf, TextRect) # Apparition du texte

        button("Quitter",50,50,100,50,ROUGE_FONCE,ROUGE, game_menu)# revient sur le menu en faisant appelle a sa fonction
        mouse = pygame.mouse.get_pos()
...
def drawScore():
        ...
        screen.blit(text,(600, 800)) # disposition en haut a droite
        text = font.render(f"Votre score: {joueur2.score}" , True, BLANC) # texte + appel du score 
        screen.blit(text,(600, 50)) # disposition en haut a droite

# Mise en place des condition du visuel de la vie : barre de vie de trois couleurs suivant l'evolution
def drawLivesP1():
        # conditions du visuelle de la barre de vie ( rectangle en haut a gauche sur l'ecran)
        ...
        else : # Barre de vie faible est rouge lorque qu'il ne rest que une seuls vie
...
class Joueur(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.image = (JOUEUR1) # appelle l'image du joueur
...
# Jeu lancé
  ...
  while jeu:
    ...
    if joueur.lives <= 0:
        ...
        gameOverP1()  # mise en place du menu game over lorsque le joueur a perdu toutes ses vies
